...
PrecipArray = array(0, c(360, 181, 1))   #Lon, Lat, Members
TemperatureArray = array(0, c(360, 181, 1))   #Lon, Lat, Members
...
ExportArray = array(-99, c(360,181,1, DaysInForecast, 3))    #Lon, Lat, Members, Days, (Precipitation, Tmax, Tmin)
  ...
  if(Variable == 'APCP_SFC_0'){
    ...
    for(FileNum in 1:length(FileCount)){    #LocalPaths at FileNum in FileCount corresponds to Timecount at FileNum in FileCount
      ...
      if((TimeCount[FileNum]-6) %% 24 == 0){
        ...
        for(Member in 1:1){      #Get a lat x lon x member array of precip values
    ...
    TemperatureArray = array(NA, c(360, 181, 1, 8))
    TimeStepCount = 0
    for(FileNum in 1:length(FileCount)){    #LocalPaths at FileNum in FileCount corresponds to Timecount at FileNum in FileCount
      ...
      if((TimeCount[FileNum]-6) %% 24 == 0){
        ...
        TimeStepCount = 0
        TemperatureArray = array(NA, c(360, 181, 1, 8))   #Lat, Lon, Members, TimeSteps
      }
      
    }
    for(Member in 1:1){    #The last value is not a 24h value, so it will not hit the record and wipe parts, so we stick it in now
      ExportArray[,,Member,ceiling((TimeCount[FileNum]-6) / 24), 2] = DailyMax(TemperatureArray[,,Member,])
      ExportArray[,,Member,ceiling((TimeCount[FileNum]-6) / 24), 3] = DailyMin(TemperatureArray[,,Member,])
    }
  }
