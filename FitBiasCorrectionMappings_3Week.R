...
if(Week3 < LastDay){  #If week 3 is later than the end of the month, we don't need to download any hindcasts for bias correction. Instead, we will write files with flat zeroes in all positions, so that no values will be corrected.
...
if(Week3 < LastDay){ #Similarly to above, don't try to read files if there's no need to download them. We can keep the bias corrections at zero - they won't do anything.
...
#In the case that no bias correction was needed, this will still generate the files so we don't run into errors. They will just have bias corrections of 0 - nothing happens.
