...
SPIForecastDir = paste0(MainDirectory, 'Indices\\SPI\\', ForecastDate, '\\0\\')
SPEIForecastDir = paste0(MainDirectory, 'Indices\\SPEI\\', ForecastDate, '\\0\\')
PDIForecastDir = paste0(MainDirectory, 'Indices\\PDI\\', ForecastDate, '\\0\\')
#ForecastDroughtFile = paste0(MainDirectory, 'Indices\\CDM_Previous\\', ForecastDate, '\\CDM.csv')
ForecastDroughtFile = paste0(MainDirectory, 'Indices\\CDM_Previous\\2020-04-02\\CDM.csv')
...
for(FileNum in 1:length(IntFiles)){
  ...
  SPIForecastDirMem = paste0(SPIForecastDir, '\\')
  SPEIForecastDirMem = paste0(SPEIForecastDir, '\\')
  PDIForecastDirMem = paste0(PDIForecastDir, '\\')
  ...
  SPIData = as.matrix(SPIData)[dim(SPIData)[1]-3,1:14]          #Only take the last month, as represented by the first index being equal to the first
  SPEIData = as.matrix(SPEIData)[dim(SPEIData)[1]-3,3:14]       #index length. The second index represents, for example, the one to twelve month SPI.
  PDIData = as.matrix(PDIData)[dim(PDIData)[1]-3,4]
...
for(TrainGroup in TrainGroups){
  ...
  Samp = sample(which(TrainMatChange[,30] == 3), size = 1500)
  SubSampleMat = rbind(SubSampleMat, TrainMatChange[Samp,])
  Samp = sample(which(TrainMatChange[,30] == 2), size = 2200)
  SubSampleMat = rbind(SubSampleMat, TrainMatChange[Samp,])
  Samp = sample(which(TrainMatChange[,30] == 1), size = 12000)
  SubSampleMat = rbind(SubSampleMat, TrainMatChange[Samp,])
  Samp = sample(which(TrainMatChange[,30] == 0), size = 10000)
  SubSampleMat = rbind(SubSampleMat, TrainMatChange[Samp,])
