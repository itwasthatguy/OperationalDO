...
MMDD4 = paste0('http://collaboration.cmc.ec.gc.ca/cmc/ensemble/subX/hindcast/', format(Week4, format='%m'), format(Week4, format='%d'), '/')
...
dir.create(paste0(MainDir, 'Temp\\Week4\\', ForecastDate, '\\'))
...
for(Rep in 1:3){              #It's likely that the network will crap out at some point and we'll lose a file or two. This will re-run all missing archives so we'll probably be good. 3 Is arbitrary, but it should work well.
  for(Year in 1998:2017){
    for(Member in 1:4){
      ...
      ArchiveString = paste0('subX_reforecast_ECCC_', Year, format(ForecastDate, format='%m'), format(Week4, format='%d'), "00_m0", Member, '.tar')
      if(!file.exists(paste0(MainDir, 'Temp\\Week4\\', ArchiveString))){
        WgetString = paste0(MMDD4, ArchiveString)
        CmdString = paste0(MainDir, 'wget.exe -r -p -nH --cut-dirs 6 -np -P ', MainDir, 'Temp\\Week4\\ robots=off ', WgetString)
        system(CmdString)
      
        RarString = paste0('winrar e -inul ', MainDir, 'Temp\\Week4\\', ArchiveString, ' ', Vars[1], ' ', MainDir, 'Temp\\Week4\\', ForecastDate)
        system(RarString)
        RarString = paste0('winrar e -inul ', MainDir, 'Temp\\Week4\\', ArchiveString, ' ', Vars[2], ' ', MainDir, 'Temp\\Week4\\', ForecastDate)
        system(RarString)
        RarString = paste0('winrar e -inul ', MainDir, 'Temp\\Week4\\', ArchiveString, ' ', Vars[3], ' ', MainDir, 'Temp\\Week4\\', ForecastDate)
        system(RarString)
      }
    }
  }
...
for(Variable in 1:3){
  ...
  Files = list.files(paste0(MainDir, 'Temp\\Week4\\', ForecastDate, '\\'), pattern = Vars[Variable], full.names=TRUE)
